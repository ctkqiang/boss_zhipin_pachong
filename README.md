...
## 合法性声明
本项目仅用于学习和研究目的，严禁用于商业用途。我们严格遵守所有适用的法律法规，包括但不限于《网络安全法》《数据安全法》以及《个人信息保护法》等。在使用本项目时，请确保遵守 BOSS 直聘网站的使用条款和 robots.txt 规则，合理控制爬取频率，避免对目标网站造成不必要的负担和干扰。

### 反爬虫策略与爬取频率控制

- **反爬虫策略**：本项目采用了一系列反爬虫策略，如模拟人类浏览行为、使用代理服务器和 IP 轮换技术等，以应对目标网站的反爬虫机制。同时，我们会不断优化这些策略，确保数据采集的合法性和稳定性。
- **爬取频率控制**：为避免对目标网站造成过大的压力，我们建议每次请求间隔不少于 1 秒。在实际运行中，会根据目标网站的响应情况动态调整爬取频率。


### 知识产权声明

- **本项目知识产权**：本项目的代码和文档受版权保护，未经授权不得用于商业用途。
- **开源代码使用**：项目中使用了一些开源代码，我们严格遵守其开源许可证的规定。具体的开源代码及其许可证信息，请参考项目中的相关文件。


### 免责声明

本项目开发者不对因用户违反法律法规、目标网站规则或本 README 中规定的使用限制而导致的任何法律责任或后果负责。用户在使用本项目时，应自行承担相应的风险。

### 数据使用限制

- **数据采集**：仅采集公开可见且允许访问的数据，不尝试绕过任何安全机制或访问受限的信息。
- **数据存储**：采集到的数据仅存储在本地的 SQLite 数据库中，用于学习和研究分析。不会将数据出售、共享或提供给任何第三方。
- **数据处理**：在处理数据时，确保数据的准确性和完整性，同时对敏感信息进行必要的脱敏处理。
